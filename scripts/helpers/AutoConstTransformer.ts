...
export function getEmittedCode(): string[] {
  return theCode;
}
...
export function collectImports(): string {
  return [...imports]
...
export class AutoConstTransformer extends BaseJavaCstVisitorWithDefaults {
  output: string[];
  curFile: string;
  ...
  transformFile(fileContents: string) {
    this.curFile = fileContents;
    this.visit(parse(fileContents));
  }

  getCode(start: number, end: number): string {
    return this.curFile.substring(start, end + 1);
  }

  getItemContent(f: unknown): string {
    if (
      hasFieldType(
        f,
        'location',
      ...
      return this.getCode(f.location.startOffset, f.location.endOffset);
    }
    return '';
  }

  getContent(field: unknown, sep?: string): string {
    return isArray(field)
      ? field.map(this.getItemContent, this).join(sep ?? ' ')
      : this.getItemContent(field);
  }
  ...
  fieldDeclaration(ctx: FieldDeclarationCtx, param?: any) {
    ...
    codeAdd(this.getContent(ctx.fieldModifier), ' ');
  ...
  unannType(ctx: UnannTypeCtx, param?: any) {
    ...
    if (ctx.unannPrimitiveTypeWithOptionalDimsSuffix) {
      codeAdd(this.getContent(ctx.unannPrimitiveTypeWithOptionalDimsSuffix));
    }
  }
  ...
  unannClassOrInterfaceType(ctx: UnannClassOrInterfaceTypeCtx, param?: any) {
    const typeName = noWhitespace(this.getContent(ctx.unannClassType));
  ...
  lambdaExpression(ctx: LambdaExpressionCtx, param?: any) {
    const params = this.getContent(ctx.lambdaParameters, ', ');
  ...
  lambdaBody(ctx: LambdaBodyCtx, param?: any) {
    const expr = noWhitespace(this.getContent(ctx.expression));
  ...
  unqualifiedClassInstanceCreationExpression(
    ...
    const typeName = this.getContent(ctx.classOrInterfaceTypeToInstantiate);
    codeAdd(' ', typeMap.get(typeName) || typeName);
  ...
  argumentList(ctx: ArgumentListCtx, param?: any) {
    ...
    ctx.expression.forEach((node, idx) => {
      const prefix = idx === 0 ? '' : ', ';
      const code = this.getContent(node);
      if (
        idx === 2 &&
